...
export vpch, vpta
    ...
    function vpta(dados, save)
        ...
        plt = plot(xGrid[:,2], yestimado, xlabel = "Diâmetro à altura do peito (cm)", ylabel = "Volume (m³)", ylim =(0, maximum(yestimado)), grid_linewidth = 0, color = "green", legend = false, ms = 6)
        plt = scatter!(dap, v, ylim = (0, maximum(yestimado)))
